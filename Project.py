...
with st.sidebar.container():
    test = st.sidebar.selectbox("Navigation", ['Home', "About Us"])

if test == "Home":
    st.title(':blue[Kalkulator Penentu Massa Unsur Dalam Suatu Senyawa]')
    st.markdown('''Hai users, selamat datang di web kami.👋😄''')
    st.markdown('''Web ini  digunakan untuk menentukan massa unsur dalam suatu senyawa() .''')
        

    tab1, tab2 =st.tabs(['Tentang Unsur','Kalkulator',])

    with tab1:
        st.header('Pengertian Unsur')
        ...
        st.write("Tabel Periodik Unsur:")
        st.write(df)
        
        #Pengertian Lambang Unsur
        st.header('Lambang Unsur')
    ...
    with tab2:
        ...
        def main():
            ...
            data1 = st.number_input("Masukkan jumlah atom unsur yang dicari:")
            data2 = st.number_input("Masukkan nilai massa senyawa:")
...
if test == "About Us":
    st.title(':blue[Mengenal Dekat Dengan Kami?]')
    ...
    st.write('Ingin tahu siapa kami, Klik tomboh dibawah yaaa')
    tombol=st.button('Pleasee Klik 🙏🥹')

    if tombol:
        st.write('Barant Gabriel Sianturi:      2360085')
        st.write('Dhiffa Zikrullah Ramadhan:    2360106')
        st.write('Elis Destriana Sari:          2360115')
        st.write('Joshua Ferdinand Immanuel:    2360153')
        st.write('Sovia Cahya Nurani:           2360267')
